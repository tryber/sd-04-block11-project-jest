...
/*
A função fetchURL retorna um JSON com informações de
um usuário aleatório buscadas da API 'randomuser.me'.
No entanto, nos testes abaixo, queremos que todas as vezes que chamarmos a API
a resposta contenha as informações do nosso adminis..Cof! Cof!.. programador favorito, Tunicão.
...
describe("verifica o usuário", () => {
  ...
  test("verifica se o usuário é o tunico", async () => {
    api.fetchURL = jest.fn().mockReturnValue(tunicoReturn);
    expect(api.fetchURL().gender).toEqual("male");
    expect(api.fetchURL().name.first).toEqual("Antônio");
    expect(api.fetchURL().name.last).toEqual("Britto");
    expect(api.fetchURL().location.country).toEqual("Brazil");
    expect(api.fetchURL().email).toEqual("tunico@bol.com.br");
    expect(api.fetchURL().login.username).toEqual("tunicao123");
    expect(api.fetchURL().login.password).toEqual("1234567890");
  });
});
